...
func TestClient_DeleteEmail(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		c := newClient()
		c.doer = mDoer
		resp, err := c.DeleteEmail(context.Background(), "test@example.com")
	...
	t.Run("error from newRequest", func(t *testing.T) {
		c := newClient()
		_, err := c.DeleteEmail(nil, "test@example.com")
	...
	t.Run("error from do", func(t *testing.T) {
		...
		c := newClient()
		c.doer = mDoer
		_, err := c.DeleteEmail(context.Background(), "nonexistent@example.com")
		require.Error(t, err)
